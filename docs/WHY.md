...
## SystemConfig + Settings UI -- Post-Phase 8
WHAT: A singleton Django model (SystemConfig) that stores API keys (VirusTotal, AbuseIPDB) with Fernet encryption, Gmail OAuth connection status, detection thresholds (clean/malicious), and fetch interval. An ADMIN-only Settings page allows configuring all of these through the browser, including a complete Gmail OAuth web flow (upload credentials, connect, disconnect).
WHY: In a real BEP deployment, an administrator must be able to configure API integrations without SSH access to the server. Hardcoding API keys in environment variables requires server-side access for every key rotation. The web OAuth flow eliminates the need to run `generate_gmail_token.py` from a terminal, making the system fully self-service. Fernet encryption ensures API keys are not stored as plaintext in the database, addressing a key security concern for BISP assessment criteria. The threshold configuration allows operators to tune detection sensitivity without code changes, which is standard practice in commercial BEP products (Group-IB, Proofpoint, Abnormal Security).
HOW IT CONNECTS: The Decider reads thresholds from SystemConfig on each analysis. TI sync tasks check `ti_sync_enabled` before running. The Gmail web flow uses `google_auth_oauthlib.flow.Flow` (web app type) instead of `InstalledAppFlow` (desktop type), with CSRF protection via session state parameter. The Settings page is accessible from the sidebar under Admin > Settings.
REPORT CHAPTER: Chapter 5, Section 5.3 (System Configuration); Chapter 6, Section 6.1 (Security Architecture)

## Production Deployment -- Post-Phase 8
WHAT: Production-ready Docker Compose configuration with gunicorn (3 workers), Caddy reverse proxy (automatic HTTPS), WhiteNoise static file serving, and a step-by-step Hetzner Cloud deployment guide.
WHY: Deploying to a real server proves the system works outside the development environment. For the BISP viva, having the system accessible via a public URL demonstrates professional-grade deployment. Caddy was chosen over nginx because it provides automatic HTTPS with zero configuration (Let's Encrypt). Gunicorn replaces Django's development server for production-grade request handling. WhiteNoise eliminates the need for a separate static file server.
HOW IT CONNECTS: `docker-compose.prod.yml` extends the existing 5-service architecture with a 6th service (Caddy). Production settings (`prod.py`) add security headers, CSRF origin checking, and cookie security. The deployment guide (`docs/DEPLOYMENT.md`) provides 10 sequential steps from server creation to running the application. The admin guide (`docs/ADMIN_GUIDE.md`) covers post-deployment configuration.
REPORT CHAPTER: Chapter 5, Section 5.4 (Deployment); Chapter 7, Section 7.1 (Deployment Architecture)
